# services/vertex.py
"""
Vertex AI helpers (ADC-based)

- Uses Application Default Credentials (ADC). Make sure you ran:
    gcloud auth application-default login

Exports:
  - init_vertex()
  - get_model(model_name: str) -> GenerativeModel
  - draft_reply(model_name: str, prompt: str) -> str
  - get_mm_model() -> MultiModalEmbeddingModel
  - get_image_embedding(image_bytes: bytes) -> list[float]
"""

from __future__ import annotations

import typing as _t
import vertexai
from vertexai.generative_models import GenerativeModel
from vertexai.vision_models import MultiModalEmbeddingModel, Image

_model: _t.Optional[GenerativeModel] = None
_mm_model: _t.Optional[MultiModalEmbeddingModel] = None


def init_vertex():
    """
    Initialize Vertex AI with ADC.
    Project/location are inferred from your environment unless explicitly set elsewhere.
    """
    vertexai.init()


def get_model(model_name: str) -> GenerativeModel:
    """
    Return a cached GenerativeModel instance for the given model name.
    Caches the last used model to avoid re-initialization overhead.
    """
    global _model
    if _model is None:
        init_vertex()
        _model = GenerativeModel(model_name)
    return _model


def draft_reply(model_name: str, prompt: str) -> str:
    """
    Generate a concise text reply from a generative model.
    Returns an empty string if the model provided no `.text`.
    """
    model = get_model(model_name)
    resp = model.generate_content(prompt)
    return getattr(resp, "text", "").strip()


def get_mm_model() -> MultiModalEmbeddingModel:
    """
    Return a cached MultiModalEmbeddingModel (multimodalembedding@001).
    """
    global _mm_model
    if _mm_model is None:
        init_vertex()
        _mm_model = MultiModalEmbeddingModel.from_pretrained("multimodalembedding@001")
    return _mm_model


def get_image_embedding(image_bytes: bytes) -> list[float]:
    """
    Compute an image embedding from raw bytes (no GCS upload).
    Returns: list[float]
    """
    model = get_mm_model()
    img = Image(image_bytes=image_bytes)
    resp = model.get_embeddings(image=img)
    # Ensure plain Python floats
    return [float(x) for x in resp.image_embedding]
