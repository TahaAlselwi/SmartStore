...
st.caption("Real-time insights powered by BigQuery AI, directly from your data.")
...
with tab_tickets:
    ...
    c2.metric("Negative Rate", f"{negative_rate:.1%}")
    c3.metric("Urgent Tickets", f"{urgent_count:,}")
    ...
    if not churn_kpis.empty:
        r = churn_kpis.iloc[0]
        avg_risk = float(r.get("avg_risk", 0.0) or 0.0)
        p90_risk = float(r.get("p90_risk", 0.0) or 0.0)
        high_risk_count = int(r.get("high_risk_count", 0) or 0)
        total_t = int(r.get("total_tickets", 0) or 0)

    c_r1, c_r2, c_r3, c_r4 = st.columns(4)
    c_r1.metric("Avg Risk", f"{avg_risk:.2f}")
    c_r2.metric("P90 Risk", f"{p90_risk:.2f}")
    c_r3.metric("High-Risk Tickets (≥0.7)", f"{high_risk_count:,}")
    c_r4.metric("Total Scored", f"{total_t:,}")
    ...
    if not dist.empty and {"bucket_label","cnt"}.issubset(dist.columns):
        ...
        st.info("No data for churn distribution.")
    ...
    if not top_risky.empty:
        # عرض المخاطر كـ نسبة مئوية اختيارياً
...
with tab_products:
    ...
    c4.metric("Abusive Comments", f"{abusive_count:,}")
    c5.metric("Abuse Rate", f"{abuse_rate:.1%}")
    c6.metric("Satisfied Rate (Products)", f"{satisfied_rate:.1%}")

    c7, c8 = st.columns(2)
    c7.metric("Satisfied Products", f"{satisfied_count:,}")
    c8.metric("Scored Products", f"{total_scored:,}")
    ...
    if not abuse_dist.empty and {"abusive","cnt"}.issubset(abuse_dist.columns):
        ...
        col1.info("No data to display.")

    # Abusive examples table
    col2.caption("Examples (Flagged as Abusive)")
    if not examples_df.empty:
        col2.dataframe(examples_df, use_container_width=True, hide_index=True)
    else:
